...
func (d *DiscussTagDao) UpdateDiscussTag(ctx context.Context, key string, discussTag *foundationmodel.DiscussTag) error {
	filter := bson.D{
		{"_id", key},
	}
	update := bson.M{
...
func (d *DiscussTagDao) GetDiscussTagList(ctx context.Context, maxCount int) ([]*foundationmodel.DiscussTag, int, error) {
	filter := bson.M{}
